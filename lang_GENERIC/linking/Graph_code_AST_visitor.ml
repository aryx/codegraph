...
let rec map_name env n : unit =
  ...
  H.when_uses_phase env (fun () ->
      ...
      let tk = H.info_of_name n in
      H.add_use_edge env n2 tk);
  match n with
  | Id (v1, v2) ->
      (* ----------- *)
      (* Boilerplate *)
...
and map_qualified_info env
  ...
  (* do we want to recurse? At least for qualifier which can contain a QExpr*)
...
and map_expr_kind env ekind : T.t option =
  ...
  | N v1 ->
      ...
      H.when_uses_phase_or_none env (fun () ->
          match L.lookup_name_and_set_resolved_if_needed env v1 with
          | Some n ->
              (* old: Some (T.N xs) *)
              L.lookup_type_of_node env n
  ...
  | DotAccess (v1, v2, v3) ->
      ...
      H.when_uses_phase_or_none env (fun () ->
          ...
          | T.TN xs, FN (Id (id, idinfo)) -> (
              let final = xs @ [ id ] in
              (let/ n2 = L.lookup_resolved_name env final in
               let tk = snd id in
               H.add_use_edge env n2 tk);
              match L.lookup_resolved_type_of_resolved_name env final with
              | Some rt ->
                  (* we set idinfo.id_type here for deeper typed metavar
                   * !!! we modify the AST here !!!
                   *)
                  idinfo.id_type := T.to_ast_type rt;
                  (* bugfix: we return rt here! not t! *)
                  Some rt
              (* in Python they use a.b.c.d for module access too *)
              | None -> Some (T.TN final))
          | _ -> None) |> H.wrap_stat env "DotAccess type"
  | Call (v1, v2) ->
...
and map_for_header env = function
  ...
  | MultiForEach v ->
      v |>
      List.iter (function
        | FE v -> map_for_header env (ForEach v)
        | FECond (v1, v2, v3) -> 
              let v1 = map_for_header env (ForEach v1) in
              let _v2 = map_tok env v2 in
...
(* Type *)
(*****************************************************************************)
and map_type_ env { t; t_attrs } : T.t option =
  let _t_attrs = map_of_list (map_attribute env) t_attrs in
  let _t = map_type_kind env t in
  H.when_uses_phase_or_none env (fun () ->
     (* TODO: move that in map_type_kind? *)
    T.of_ast_type env.file_or_package_qualifier { t; t_attrs }
  )

and map_type_kind env v =
  match v with
  (* ----------- *)
  (* Boilerplate *)
  (* ----------- *)
...
and map_definition env (ent, def) =
  let env =
    ...
    | Some id -> (
        ...
        (* TODO? update_resolved_if_none ent.name? *)
        let entname = N.to_entname dotted_ident in
        ...
        | ClassDef classdef ->
            ...
            (* For Ruby, Python. See the comment in T.of_expr_after_naming *)
            | { cextends = [ ({ t = TyExpr { e = N parent; _}; _ }, None) ];_ }
             ->
                H.when_uses_phase env (fun () ->
                    (* !!the uses!! *)
                    let/ n2 = 
                      L.lookup_name_and_set_resolved_if_needed env parent in
